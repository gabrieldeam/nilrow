...
import { getCatalogByCatalogId, editCatalog } from '../../../services/catalogApi';
import { getAddressById } from '../../../services/profileApi'; // Importando o serviço para buscar o endereço
...
const EditCatalog = () => {
        ...
        if (id) {
            ...
            getCatalogByCatalogId(id).then(async (catalog) => {
                ...
                const orderedOperatingHours = fullDaysOfWeek.map(day => {
                    ...
                    return dayData ? {
                        openCloseTimes: dayData.timeIntervals.length > 0 ? dayData.timeIntervals.map(time => ({ open: time.openTime, close: time.closeTime })) : [{ open: '', close: '' }],
                ...
                if (orderedOperatingHours[0].openCloseTimes.some(time => time.open || time.close) || orderedOperatingHours[0].is24Hours || orderedOperatingHours[0].isClosed) {
                    setSelectedDay(0); 
                }
        ...
        if (validateForm()) {
            ...
            try {
                ...
                if (error.response && error.response.data) {
                    const errorMessage = error.response.data.message || 'Erro ao editar o catálogo. Tente novamente.';
    ...
    return (
        <div className="add-catalog-page">
            ...
            {isMobile && (
                <MobileHeader title="Editar Catálogo" buttons={{ close: true }} handleBack={handleBack} />
            )}
            <div className="add-catalog-container">
                <SubHeader title="Editar Catálogo" handleBack={handleBack} />
