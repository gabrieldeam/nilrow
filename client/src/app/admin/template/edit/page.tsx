...
import {
  ...
  listVariationTemplateImagesByVariation,
  updateVariationTemplateImage,
  createVariationTemplateImage,
  deleteVariationTemplateImage,
...
import {
  ProductTemplateDTO,
  ProductType,
  TechnicalSpecificationTemplateDTO,
  ProductTemplateVariationDTO,
  VariationTemplateAttributeDTO,
} from '@/types/services/productTemplate';
import { VariationTemplateImageDTO } from '@/types/services/productTemplate';
...
const ProductEdit = () => {
  ...
  const [initialVariations, setInitialVariations] = useState<ProductTemplateVariationDTO[]>([]);
  ...
  // 4) Ficha técnica
  const [technicalSpecifications, setTechnicalSpecifications] = useState<TechnicalSpecificationTemplateDTO[]>([]);

  // 5) Atributos e variações
  const [productAttributes, setProductAttributes] = useState<ProductAttribute[]>([]);
  const [variations, setVariations] = useState<ProductTemplateVariationDTO[]>([]);
  ...
  useEffect(() => {
    async function fetchProduct() {
      if (!productId) return;
      try {
        const product: ProductTemplateDTO = await getProductTemplateById(productId);
        ...
        if (product.variations && product.variations.length > 0) {
          for (let i = 0; i < product.variations.length; i++) {
            ...
            const backendImages = await listVariationTemplateImagesByVariation(varItem.id);
            varImagesMap[i] = backendImages.map((variationImage: VariationTemplateImageDTO) => ({
  ...
  const handleDelete = async () => {
    if (!productId) return;
    try {
      await deleteProductTemplate(productId);
  ...
  const generateVariations = useCallback(() => {
    ...
    const createVariationKey = (attributes: VariationTemplateAttributeDTO[]): string => {
    ...
    const areAttributesSimilar = (
      attrs1: VariationTemplateAttributeDTO[],
      attrs2: VariationTemplateAttributeDTO[]
    ): boolean => {
    ...
    const variationMap = new Map<string, ProductTemplateVariationDTO>();
    ...
    const newVariations: ProductTemplateVariationDTO[] = combos.map(combo => {
      const attributes: VariationTemplateAttributeDTO[] = combo.map((val, idx) => ({
  ...
  const handleChangeVariationField = (
    variationIndex: number,
    field: keyof ProductTemplateVariationDTO,
  ...
  const handleSubmit = useCallback(
    async (event: React.FormEvent) => {
      ...
      const updatedProduct: ProductTemplateDTO = {
      ...
      try {
        ...
        await updateProductTemplate(productId!, updatedProduct, newMainImageFiles);
        ...
        for (let varIndex = 0; varIndex < variations.length; varIndex++) {
          ...
          const serverImages = await listVariationTemplateImagesByVariation(variationId);
          ...
          await Promise.all(deletedIds.map((delId) => deleteVariationTemplateImage(delId)));
          ...
          await Promise.all(
            currentImages.map(async (imgData, i2) => {
              ...
              if (!imgData.id) {
                if (imgData.file) {
                  await createVariationTemplateImage(varData.id!, imgData.file!, newOrderIndex);
                }
              } else {
                ...
                if (needsFileUpdate || needsReorder) {
                  await updateVariationTemplateImage(imgData.id!, imgData.file, newOrderIndex);
                }
              }
            })
          );
    ...
    <div className={styles.productPage}>
      ...
      {isMobile && (
        <MobileHeader
          ...
          buttons={{ close: true }}
          handleBack={() => router.push('/admin/template')}
        />
      )}
