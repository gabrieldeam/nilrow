...
import { createProductTemplate, createVariationTemplateImage } from '@/services/product/productTemplateService';
import {
...
const ProductCreateContent: React.FC = () => {
  ...
  const generateVariations = useCallback(() => {
    ...
    const createVariationKey = (attributes: VariationTemplateAttributeDTO[]) => {
    ...
    const newVariations: ProductTemplateVariationDTO[] = combos.map(combo => {
      ...
      return {
        id: undefined,
        name: '',
        price: 0,        // Valor inicial (campo obrigatório)
        discountPrice: 0,
        stock: 0,        // Valor inicial (campo obrigatório)
        active: true,
        images: [],
        attributes,
      };
  ...
  const handleChangeVariationField = (
    ...
    setVariations((prev) => {
      const arr = [...prev];
      arr[variationIndex] = { ...arr[variationIndex], [field]: String(value) };
      return arr;
    });
  };
  ...
  const handleSubmit = useCallback(
    async (event: React.FormEvent) => {
      ...
      // Alteração na validação: não usamos !salePrice nem !stock para valores numéricos,
      // assim 0 é considerado válido
      if (
        name.trim() === '' ||
        categoryId.trim() === '' ||
        brandId.trim() === ''
      ...
      const newProduct: ProductTemplateDTO = {     
        images: [],
        name,
        type,
        categoryId,
      ...
      try {
        ...
        const createdProduct = await createProductTemplate(
          newProduct,
            ...
            createdProduct.variations.map((createdVar, varIndex) => {
              ...
              return Promise.all(
                imagesForThisVar.map((imgData, imgIndex) =>
                  createVariationTemplateImage(createdVar.id!, imgData.file!, imgIndex)
                )
              );
            })
          );
